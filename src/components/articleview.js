...
const ArticleView = (article) => (
	<div className={`${article.order%2 ? 'bg-gray-200' : ''}`}>
		<div className="container mx-auto mt-6 mb-10 px-5 py-12">
			<Link to={`/content/${article.slug}`} className="hover:text-purple-700">
				<h3 className="font-medium text-3xl">{article.title}</h3>
			</Link>
			<div className="text-gray-600 text-xl mt-3 mb-4" dangerouslySetInnerHTML={{ __html: article.summary }}></div>
			{/* {JSON.stringify(index)} */}
			<div className={`flex mt-5 ${article.order%2 ? 'flex-row-reverse ' : ''}`}>
				<div className="w-1/2 content">
					<div className="text-gray-600 text-lg rounded-lg border-red-200 p-4 pl-5 shadow-md bg-white mb-5 content">
						<div className="text-red-500 -m-t-1 mb-1 uppercase font-semibold text-sm">Problem</div>
						<div  dangerouslySetInnerHTML={{ __html: article.case_study.problem }}></div>
					</div>

					<div className="text-gray-600 text-lg rounded-lg border-red-200 p-4 pl-5 shadow-md bg-white mb-5 content">
						<div className="text-orange-500 -m-t-1 mb-1 uppercase font-semibold text-sm">Solution</div>
						<div  dangerouslySetInnerHTML={{ __html: article.case_study.solution }}></div>
					</div>

					<div className="text-gray-600 text-lg rounded-lg border-red-200 p-4 pl-5 shadow-md bg-white mb-5 content">
						<div className="text-green-500 -m-t-1 mb-1 uppercase font-semibold text-sm">Impact</div>
